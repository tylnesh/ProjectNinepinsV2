...
void FullGameWindow::onRedrawGUI()
{
  ...
  //  }
   ...
   // timehelper++;
   // if (timehelper > 100) { sndScore(); timehelper = 0;}
...
void FullGameWindow::handleTimeout()
{
    if (m_readData.isEmpty()) {
        ...
        if (QString(m_readData.mid(14,1)).toInt() == 0) {
        if (QString(m_readData.mid(15,1)).toInt() == 0) {
            F_incoming.score = QString(m_readData.mid(16,1)).toInt();
        }
            else F_incoming.score = QString(m_readData.mid(15,2)).toInt();
        }
        else F_incoming.score = QString(m_readData.mid(14,3)).toInt();
        ...
        int inChecksum = 0;
        if (QString(m_readData.mid(17,1)).toInt() == 0) {
        if (QString(m_readData.mid(18,1)).toInt() == 0) {
            inChecksum = QString(m_readData.mid(19,1)).toInt();
        }
            else inChecksum = QString(m_readData.mid(18,2)).toInt();
        }
        else inChecksum = QString(m_readData.mid(17,3)).toInt();

        qDebug() << "Incoming checksum is: " << inChecksum;

        if (checksum != inChecksum) { cmdOutGF = 6; sndMsg(); cmdOutGF = 1; }

        if (F_incoming.wire == WIRE && F_incoming.cmd == 1) { parseMsg(); if (roundsGF != 0) {sndScore(); sndScore();}}
        if (F_incoming.wire == WIRE && F_incoming.cmd == 100) { isEnding = true; qDebug() << "Arduino is shutting down";}
    ...
    //if (F_incoming.wire == WIRE){
             ...
             //{
                ...
                // if (!isPartial) {
                     ...
                     scoreGF = F_incoming.score;



                     for (int i = 0; i<9; i++) {
                     pinsOutGF[i] = pinsGF[i];
                                          }
                     pointsOutGF = pointsGF;
                     roundsOutGF = roundsGF;
                     scoreOutGF= scoreGF;
...
void FullGameWindow::sndMsg(){
    ...
    checksum += WIRE + F_outgoing.cmd;
    for (int i = 0; i<9; i++) checksum += F_outgoing.pins[i];
    checksum += F_outgoing.rounds + F_outgoing.score;

    QByteArray w,c,r,s,ch;
    QByteArray p[9];
    ...
    s.append(F_outgoing.score);
    ch.append(checksum);

    qDebug() << w<<c;
    for (int i = 0; i<9; i++) qDebug()<<p[i];
    qDebug() << r << s;
