...
uint8_t SIMBUTTONS[10] = {34, 36 , 38, 40, 42, 44, 46, 48, 50, 52};
...
uint8_t LED[9] = {12, 11, 10, 9, 8, 7, 6, 5, 4};
...
receiveMessage();
  for (int i = 0;  i < 10; i++)
{
  //Serial.print("Digital pin ");  Serial.print(i); Serial.print(": "); Serial.println(digitalRead(SIMBUTTONS[i]));
  if (i<9){
  if (digitalRead(SIMBUTTONS[i])) analogWrite(LED[i],HIGH);
   else { analogWrite(LED[i],LOW);}
  } else
  {
      if (digitalRead(SIMBUTTONS[i])) {
        analogWrite(LED_ERROR, HIGH);
        analogWrite(LED_START, HIGH);
        }
      else
      {
