...
String program_version = "2019-05-24";
//WIRE
#define WIRE 1
...
Thread test1Thread = Thread();
  Thread test0Thread = Thread();
...
void setup() {
 ...
 Serial1.begin(57600);
    ...
    commThread.onRun(comm);
    commThread.setInterval(100);

    // Configure gameThread
    gameThread.onRun(game);
    gameThread.setInterval(10);


    test1Thread.onRun(test1);
    test1Thread.setInterval(1500);
    test0Thread.onRun(test0);
    test0Thread.setInterval(1000);
  
   
  // Adds myThread to the control
  tc.add(&gameThread);
  tc.add(&commThread);
  tc.add(&test1Thread);
    tc.add(&test0Thread);
...
void test1() {
  analogWrite(LED_CARD,brightness);
  Serial.println("Test1 thread");
...
void test0() {
  analogWrite(LED_CARD,LOW);
  Serial.println("Test2 thread");
    Serial.println(millis());
  }
...
int checkPins() {
    ...
    noneFallen = false;
    int val = 0;// Indescriptive naming
    for (int i = 0; i < 9; i++)
    {
...
int checkLED()
  {
    Serial.println("************KOTNROLA LED*************"); // Create a function that prints this, but only if some macro is set to enable all debugging mersages
    for (int i = 0; i < 9; i++) {
...
void lowerPins() {
  ...
  void jumpOut() {
    if (incoming.cmd == -100) { resetIncoming(); loop();} // Split to more lines
    }
...
bool checkError()
  {
    ...
    Serial.print("docasnepins: ");
    for (int i = 0; i<9; i++) Serial.println(docasnepins[i]);
    
    
    analogWrite(LED_ERROR, brightness);
      while (true) {

        if (digitalRead(GATE_RAMP) == HIGH) {  //pozor, tento senzor ma opacnu logiku
          gateSensorCounter++;

          Serial.println("Presla gula!"); analogWrite(LED_START, 0); //break; //ak presla gula, pokracujeme kontrolovanim padu kolkov
        }
        if (digitalRead(GATE_RAMP) == LOW) { //pozor, tento senzor ma opacnu logiku ## ak nepride opakovany signal prechodu gule, tak mohla nastat chyba senzora, udaje vynulujeme a snimame dalej
          gateSensorCounter = 0;
 ...
 showFallenPins();
        ...
        if (incoming.cmd == 206) {
         ...
         if (gameType == 1) { resetPinCount(); } else  { for (int i =0; i<9; i++){ 
          toMAG[i] = docasnepins[i]; pins[i] = docasnepins[i];
          outgoing.pins[i] = pins[i];} } // Endbrace to next line
      ...
      while (true)
      {
        ...
        if (incoming.cmd == 101)
          ...
          for (int i =0; i<9; i++) {
            ...
            pins[i] = incoming.pins[i];  
            
            toMAG[i] = pins[i]; showFallenPins();
            
            }
          
          
          //fallenCounter = incoming.points;
          Serial.println("pins "); for (int i =0; i<9; i++) Serial.print(incoming.pins[i]);
...
void offLED() { for (int i = 0; i<9; i++) analogWrite(LED[i],0); // Give a name to number 9
  }
...
void resetIncoming(){
  incoming.cmd = 0; incoming.wire = 0; // Use newlines more generously!!!
...
void resetGame(){
  ...
  for (int i=0;i<9;i++) {prevPins[i] = 0; fallen[i] = 0; toMAG[i] = 0; pins[i] = 0;}
   ...
   void resetPinCount()
  { // Unite into one functions that takes the array to be zeroed as argument
...
for (int i =0; i<9; i++) Serial.println(incoming.pins[i]);
...
void readMsg(){
      if (incoming.wire == WIRE) {
        ...
        switch (incoming.cmd){
          ...
          analogWrite(LED_START, LOW);
          newState = setState();
          if(newState) analogWrite(LED_ERROR, brightness);
          analogWrite(LED_START, brightness);
          resetIncoming();
          break;
